...
export class ApresentadorOid extends OidUI {
  handleGetJsonHTMLDescription(topic, message) {
    ...
    document.querySelectorAll("button-oid").forEach(button => button.style.display = "none")
    ...
    for (let node of json) {
      ...
      let nodeHTMLparams = node.params.map(param => `${param.name}="${param.value}"`).join(" ").replace(/"/g, "'")
